...
@Component({
  ...
  styleUrls: ['./productreactiveform.view.css']
})
export class ProductReactiveFormComponent implements OnInit {
  ...
  save(): void {
    ...
    if (this.isUniqueProductId(parseInt(this.frmPrd.value.ProductId))) {
      this.product = this.frmPrd.value;
      this.products = this.logic.saveProducts(this.product);
      console.log(JSON.stringify(this.products));
      this.frmPrd.controls['ProductId'].setErrors(null);
    }
    else {
      this.frmPrd.controls['ProductId'].setErrors({ 'nonUniProductId': true });
    }
  ...
  public isUniqueProductId(prdId: number) {
    return this.products.find(p => p.ProductId === prdId) === undefined;
  }

  public onCategorySelected(event: string) {
    this.frmPrd.value.Category = event;
  }
}
